...
def Est_LcpH(Z_train,X_train,U_train,De_train,Z_2_train,X_test,Z_2_test,Theta0,nodevec,m,c0):
    Lambda_U = I_S(m,c0,U_train,nodevec)
    C_index = 0
    zeta0 = np.mean(Z_2_train)
    for loop in range(100):
        Res_g = g_LCPH(De_train,Z_train,Z_2_train,X_train,X_test,Z_2_test,Lambda_U,Theta0,zeta0)
        Theta1 = Res_g['Theta']
        g_train = Res_g['g_train']
        g_test = Res_g['g_test']
        h_train = Res_g['h_train']
        ...
        Lambda_U = I_S(m,c1,U_train,nodevec)
        #Theta1 = Theta_est(De_train,Z_train,Z_2_train,Lambda_U,Res_train,zeta0)
        zeta1 = zeta_est(De_train,Z_train,Z_2_train,Res_train,Lambda_U,Theta1,seq=1)
        if (np.max(abs(Theta0-Theta1)) <= 0.01):
            C_index = 1
            break
        c0 = c1
