...
def generate_case_1(n, corr, Theta, zeta):
    ...
    U = uniform_data(n, 0, 1) #set tau=1
...
def generate_case_11(n, corr, Theta, zeta):
    Z = ndm.binomial(1, 0.5, n) #parametric
    Z_2 = ndm.normal(loc=2, scale=1, size=n) #change point
    Z_2 = np.clip(Z_2, 1.5, 2.5)
    ZC = np.vstack((Z, Z*(Z_2>zeta)))
    ...
    g_X_C = g_X+h_X*Ind
    Y = ndm.rand(n)  #U(0,1)
    T = (-5 * np.log(Y) * np.exp(-ZC.T @ Theta - g_X_C)) ** 2 #Lambda(t)=np.sqrt(t)
    U = uniform_data(n, 0, 10) #set tau=1
    De = (T <= U)  #censoring indicator
    return {
        'Z': np.array(Z, dtype='float32'),
        'X': np.array(X, dtype='float32'),
